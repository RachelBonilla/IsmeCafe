...
namespace DA
{
    public class PedidoDA : IPedidoDA
    {
        ...
        public async Task<PedidoResponse> Confirmar(Guid idUsuario, bool usarPuntos = false)
        {
            var idPedido = Guid.NewGuid();

            var cabecera = await _sqlConnection.QueryFirstAsync<(int NumeroPedido, decimal Total, DateTime Fecha, int IdEstado, string Estado, int PuntosGanados, int PuntosUsados, decimal DescuentoPuntos)>(
                "ConfirmarPedido",
                new { IdUsuario = idUsuario, IdPedido = idPedido, UsarPuntos = usarPuntos },
            ...
            return pedido ?? new PedidoResponse
            {
                ...
                IdEstado = cabecera.IdEstado,
                Estado = cabecera.Estado,
                Total = cabecera.Total,
                PuntosGanados = cabecera.PuntosGanados,
                PuntosUsados = cabecera.PuntosUsados,
                DescuentoPuntos = cabecera.DescuentoPuntos
            };
        }
        ...
        {
            var resultado = await _sqlConnection.QueryAsync<PedidoEstadoResponse>(
                "ObtenerPedidosEstados",
                commandType: CommandType.StoredProcedure);
        ...
        {
            var resultado = await _sqlConnection.QueryFirstAsync<PedidoResponse>(
