...
namespace API.Controllers
{
    ...
    public class CarritoController : ControllerBase, ICarritoController
    {
        ...
        [HttpGet("{idUsuario}")]
        public async Task<ActionResult> Obtener([FromRoute] Guid idUsuario, [FromQuery] bool usarPuntos = false)
        {
            try
            {
                var carrito = await _carritoFlujo.Obtener(idUsuario, usarPuntos);
                return Ok(carrito);
            }
            catch (Exception ex)
            {
