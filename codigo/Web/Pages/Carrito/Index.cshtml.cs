...
namespace Web.Pages.Carrito
{
    public class IndexModel : PageModel
    {
        ...
        {
            // HU-21 CA2: si el usuario no ha iniciado sesión debe ser redirigido al login.
        ...
        public async Task<IActionResult> OnPostConfirmarAsync()
        {
            ...
            try
            {
                var pedido = await _pedidoReglas.Confirmar(new PedidoRequest { IdUsuario = idUsuario });
        ...
        private async Task CargarCarrito(Guid idUsuario)
        {
            try
            {
                Carrito = await _carritoReglas.Obtener(idUsuario);
            }
            catch (Exception ex)
            {
