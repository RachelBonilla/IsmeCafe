...
namespace Web.Pages.Servicios.Admin
{
    public class GestionServiciosModel : PageModel
    {
        ...
        public GestionServiciosModel(IServicioReglas servicioReglas, ILogger<AdminModel> logger)
        {
        ...
        [BindProperty]
        public ServicioRequest Entrada { get; set; } = new() { Activo = true };

        // Cuando tiene valor, el formulario está en modo edición de ese servicio.
        [BindProperty]
        public Guid? EditId { get; set; }

        public IReadOnlyList<ServicioResponse> Servicios { get; private set; } = new List<ServicioResponse>();
        ...
        {
            new SelectListItem("Cata de cafe",  "B0000000-0000-0000-0000-000000000001"),
        ...
        public async Task OnGetAsync(Guid? id)
        {
            ...
            if (id.HasValue)
            {
                var servicio = Servicios.FirstOrDefault(p => p.Id == id.Value);
                if (servicio != null)
                {
                    EditId = servicio.Id;
        ...
        public async Task<IActionResult> OnPostGuardarAsync()
        {
            ...
            try
            {
                ...
                if (EditId.HasValue)
                {
                    ok = await _servicioReglas.Editar(EditId.Value, Entrada);
                    accion = "actualizado";
                }
                else
                {
                ...
                if (ok)
                {
                    Mensaje = $"Servicio \"{Entrada.Nombre}\" {accion} correctamente.";
                    EsExito = true;
                    ModelState.Clear();
                    Entrada = new ServicioRequest { Activo = true };
                    EditId = null;
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                Mensaje = "No fue posible completar la operación. Verifica la conexión con el API de Servicios.";
                EsExito = false;
            }
        ...
        private async Task CargarServiciosAsync()
        {
            try
            {
                ...
                Servicios = lista.OrderBy(p => p.Nombre).ToList();
            }
            catch (Exception ex)
            {
                ...
                {
                    Mensaje = "No fue posible cargar la lista de servicios. Verifica la conexión con el API.";
                    EsExito = false;
                }
            }
        }
